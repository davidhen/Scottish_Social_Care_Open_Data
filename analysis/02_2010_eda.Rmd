...
**Needs re calculated or replotted** Shows proportion of all men and all women, Not age group.

```{r la_age_freq, fig.height=12, fig.width=16}

x %>% 
  filter(!is.na(gender) &  !is.na(age_grp)) %>% 
  ggplot(aes(age_grp, frq, colour = gender)) +
  geom_pointrange(aes(ymin=0, ymax = frq), 
                  position = position_dodge(width = 0.5)) +
  facet_wrap(~council) +
  legend_top +
