...
This needs done for all the home care hour types...

```{r factorise_bandHRSLA}
#2010
soc_care10$la_hrs <- factor(soc_care10$la_hrs,      #Local authority provided
                               levels = hrs_levels, #hours
                               labels = hrs_labels)

soc_care10$pri_hrs <- factor(soc_care10$pri_hrs,    #Private company provided
                               levels = hrs_levels, #hours
                               labels = hrs_labels)

soc_care10$vol_hrs <- factor(soc_care10$vol_hrs,   #Voluntary sector provided
                               levels = hrs_levels,#hours
                               labels = hrs_labels)

soc_care10$pc_hrs <- factor(soc_care10$pc_hrs,     #Personal care hours
                               levels = hrs_levels,
                               labels = hrs_labels)
#2011
...
Sanity check on the Local Authority provided hours
...
Check on the private provided hours
...
Factorise `hc_client` This is a derived variable indicating whether client recieved home care or not. Useful for subsetting.
...
```{r drop_iorn}
soc_care10 %<>% select(-iorn_score)

soc_care11 %<>% select(-iorn_score)

soc_care12 %<>% select(-iorn_score)
```
...
One last adjustment to make - 2012 data has a variable `new_hrs` which (correctly) has been left as an integer. However if an individual has no new home care hours a `NA` value is recorded. I'll assign `NA` values to `0`.

```{r new_hrs}
soc_care12$new_hrs %<>% replace_na(0)
...
save(soc_care10, file = "C://GitHub/Scottish_Social_Care_Open_Data/output/clean_data/soc_care10.RData")
save(soc_care11, file = "C://GitHub/Scottish_Social_Care_Open_Data/output/clean_data/soc_care11.RData")
save(soc_care12, file = "C://GitHub/Scottish_Social_Care_Open_Data/output/clean_data/soc_care12.RData")
```
